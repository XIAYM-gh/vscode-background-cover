...
export class PickList {
	...
	public static createItemList() {
		...
		let items: ImgItem[] = [
			...
			{
				label: '$(pencil) Input Image URL',
				description: '输入图片路径 (Local Path / HTTPS)',
				imageType: 6
			},
			{
				label: '$(eye-closed) Disable Background',
				description: '关闭背景图',
				imageType: 7
			}
		];
	...
	public static needAutoUpdate(config: WorkspaceConfiguration) {
		...
		PickList.itemList
			.updateDom(false, nowBlenaStr as string)
			.then(() => {
	...
	public static gotoFilePath(path?: string) {
		...
		let tmpUri: string = path;
		let extPath = extensions.getExtension('xiaym-gh.background-cover')?.extensionPath;
	...
	private showInputBox(type: number) {
		...
		window.showInputBox(option).then(value => {
			...
			if (type === 1) {
				...
				if (!fsStatus && !isUrl) {
					window.showWarningMessage('Permission denied or the file does not exist! / 无权限访问或文件不存在！');
					return false;
				}
			} else if (type == 2) {
				...
				if (blur < 0 || blur > 100 || isNaN(blur)) {
					window.showWarningMessage('Blur strength ranges in 0 - 100!');
					return false;
				}
			}
	...
	public updateBackgound(path?: string) {
		if (!path) {
			return vsHelp.showInfo('Unable to fetch image path / 未获取到图片路径');
		}
	...
	private async updateDom(uninstall: boolean = false, colorThemeKind: string = ''): Promise<void> {
		...
		try {
			...
			if (result) {
				if (this.quickPick) {
					this.quickPick.placeholder = 'Reload now to make effect? / 重新加载生效？';
					...
					if (this.randUpdate) {
						// 提醒，即将自动重启生效背景
						window.showInformationMessage(`背景将在 1 秒后自动更新！ / The background will be automatically updated in 1 second!`);
						// sleep 1s
